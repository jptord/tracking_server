...
class AtxUpdater {
    ...
    restoreData(req,res,callback){ 
        ...
        folderData.forEach(f=>cmds.push(`sshpass -p scp -r ${scpData.user}@${scpData.host}:${scpData.base}/${app.name}_${isodate}_data/${f.replaceAll("./","")} ${scpData.pass} ${f}`));
        filesData.forEach(f=>cmds.push(`sshpass -p scp ${scpData.user}@${scpData.host}:${scpData.base}/${app.name}_${isodate}_data ${scpData.pass} ${f}`));        
